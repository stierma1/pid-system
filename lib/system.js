...
class System{
  ...
  static async promote(func, mode, dict, views){
    var dict = dict || {};
    dict._mode = mode || "receiver-in-message";
    return System.spawn("internal", func, dict, views);
  }
  ...
  static receiveWatch(pid, watchPids, states, options, func){
  ...
  static async exit(pid, state, reason){
    ...
    if(state === undefined){
      return pid._exit("normal", undefined, true);
    }
    return pid._exit(state,reason, true);
  }
...
class Pid extends EventEmitter{
  constructor(config){
    super(config)
  ...
  processDelayed(){
    this.delayedMailbox.map((message) =>{
  ...
  process(){
    ...
    try{
      if(GroupControls[this._module] === this._func){
        ...
        return (async function promotedFunction(){
          if(self.state !== "up"){
            return;
          }
          if(self.dictionary._mode === "emitter"){
            var message = await System.receive(self);
            if(self.state !== "up"){
              return;
            }
            var out = await this._func.apply(self, message);
            self.emit("data", out);
          } else {
            var [receiver, ...message] = await System.receive(self);
            if(self.state !== "up"){
              return;
            }
            var out = await this._func.apply(self, message);
          ...
          return System.recurse(self, promotedFunction);
        }).call(this)
          .then(function(ret){
            if(!self.keepAlive && !self.dictionary.exitExplicit && self.state === "up"){
              System.exit(self, "normal");
  ...
  _exit(state, reason, catchProcess){
    this.state = state;
    this.reason = reason;
...
var GroupControls = {
  ...
  race: async function() {
    ...
    racers.map(async (pid) => {
      var returnPid = await System.spawn("_receiver", GroupControls._receiver, {exitExplicit:true})
...
System.GroupControls.all = async (caller, pids, options) => {
  ...
  if(pids.length === 0){
   ...
   var allPid = await System.spawn("all", GroupControls.all);
...
System.GroupControls.allAsync = (message, pids, options) => {
  return new Promise(async (res, rej) => {
    ...
    var promisedPid = await System.spawn("_promised", GroupControls._promised, {exitExplicit:true});
...
System.GroupControls.fallback = async (caller, pids, options) => {
  ...
  if(pids.length === 0){
   ...
   var fallBackPid = await System.spawn("fallback", GroupControls.fallback);
...
System.GroupControls.race = async (caller, pids, options) => {
  ...
  if(pids.length === 0){
   ...
   var racePid = await System.spawn("race", GroupControls.race, {debug:false});
...
System.GroupControls.random = async (caller, pids, options) => {
  ...
  if(pids.length === 0){
   ...
   var randomPid = await System.spawn("fallback", GroupControls.fallback, {debug:false});
